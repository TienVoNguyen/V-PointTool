...
public class MarkController {
    ...
    public ResponseEntity<?> addMarkUser(@Valid @RequestBody AddMarkUser markUser,
        ...
        if (markUser.getKpiID() != null) {
            float pointKpi = markKPI(markUser.getKpiID(), markUser.getKpi(), user, localDate);
            markResponse.setPointKPI(pointKpi);
        }
        if (markUser.getBestDepartmentID() != null) {
            float pointBestDepartment = markBestDepartment(markUser.getBestDepartmentID(),
        ...
        if (markUser.getBcsDepartmentID() != null) {
            float pointBCSDepartment = markBCSDepartment(markUser.getBcsDepartmentID(),
        ...
        if (markUser.getJointActivitiesID() != null) {
            float pointJoint = markJointActivities(markUser.getJointActivitiesID(),
                    markUser.getJointActivities(), user, localDate);
            markResponse.setPointJointActivities(pointJoint);
        }
        if (markUser.getTrainID() != null) {
            float pointTrain = markTrain(markUser.getTrainID(), markUser.getTrain(), user, localDate);
            markResponse.setPointTrain(pointTrain);
        }
        if (markUser.getTrainStaffID() != null) {
            float pointTrainStaff = markTrainStaff(markUser.getTrainStaffID(),
                    markUser.getTrainStaff(), user, localDate);
            markResponse.setPointTrainStaff(pointTrainStaff);
        }
        if (markUser.getTrainVmgID() != null) {
            float pointTrainVmg = markTrainVmg(markUser.getTrainVmgID(), markUser.getTrainVmg(), user,
                    localDate);
            markResponse.setPointTrainVmg(pointTrainVmg);
        }
        if (markUser.getLoveVmgID() != null) {
            float pointLoveVmg = markLoveVmg(markUser.getLoveVmgID(), markUser.getLoveVmg(), user, localDate);
            markResponse.setPointLoveVmg(pointLoveVmg);
        }
        if (markUser.getDisciplineBonusID() != null) {
            float pointDisciplineBonus = markDisciplineBonus(markUser.getDisciplineBonusID(),
        ...
        if (markUser.getDisciplineViolateID() != null) {
            float pointDisciplineViolate = markDisciplineViolate(markUser.getDisciplineViolateID(),
