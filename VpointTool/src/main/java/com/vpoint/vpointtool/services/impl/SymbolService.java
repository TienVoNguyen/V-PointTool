...
public class SymbolService implements ISymbolService {
    ...
    @Override
    public Symbol findSymbolByItemAndName(Item item, String name) {
        return symbolRepository.findSymbolByItemAndName(item, name);
    }

    @Override
    public List<Symbol> findAllSymbolByItem(Item item) {
        return symbolRepository.findSymbolsByItem(item);
    }
}
